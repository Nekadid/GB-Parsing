...
df_vacancies.to_sql(con=engine, name='vacancies', if_exists='replace', index = False)

# for vacancy in vacancies.filter(vacancy.top_salary > 100000):
#     print(user.name, user.password)

print(engine.execute('select name from vacancies'))
session.commit()
session.close()